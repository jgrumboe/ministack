...
- **ECS — only an `awsvpc` task reports an elastic network interface** — every task got an `ElasticNetworkInterface` attachment once its container was up, whatever its network mode, while the Task reference describes `attachments` as the adapter a task has "if the task uses the `awsvpc` network mode": a `bridge` or `host` task on AWS reports none and is reached through its container instance. A task in those modes now reports an empty list, an `awsvpc` task's attachment additionally names the subnet the request placed it in, and the address stays available to the target-group sync in every mode. Contributed by @iot-rocket.
- **ECS — a task no longer reports `attachmentsStatus`** — the task record carried an `attachmentsStatus` member next to its attachment, and there is no such member on the `Task` shape: the API model has it on `Cluster` only, and a live task on AWS, awsvpc and bridge alike, sends no such field. A client reading the raw wire saw an invented member, and the emulator was the only place it existed. The cluster's own `attachmentsStatus` is untouched. Contributed by @iot-rocket.
- **ECS — the task metadata endpoint reports the status the task is in** — `${ECS_CONTAINER_METADATA_URI_V4}` served `DesiredStatus` and `KnownStatus` as the literal `RUNNING`, captured when the container was registered, so a container that read its own metadata was told the task was running while `DescribeTasks` reported `PENDING` or `ACTIVATING`. The two could not disagree before `RunTask` returned asynchronously; now that the image pull sits inside that window, they disagree for the length of it. Both members are read off the task record on every request, on the container route and the task route, and a task that no longer has a record keeps what was registered. `DesiredStatus` is the task's everywhere, which is how a container learns it is being shut down; `KnownStatus` is the task's on the task payload and the container's own on a container payload, because that is what AWS reports: a live Fargate task served `"KnownStatus": "NONE"` for itself and `"RUNNING"` for the container reading the endpoint, in one payload. The task is looked up by the account and region in its ARN, since a container reaches the endpoint with a path token and no SigV4. Contributed by @iot-rocket.
...
- **Test harness — the suite runs on a non-default `GATEWAY_PORT`** — eight tests compared a value a service had built from `GATEWAY_PORT` against a hard-coded `4566`, so the suite passed only on the default port. That is the port a contributor moves off to run a private server rather than resetting a shared one, which is exactly when the failures appear and look like real regressions. The ESM tests now key their queues through `sqs._queue_url()`, the AppSync and Glue assertions read the port from one place in `tests/conftest.py`, and the three files pass on any port. No user-visible behavior changes. Contributed by @iot-rocket.
